...
function get_details() {
    ...
    if (!$debt_obj->get_detail($student_id)) {

        echo "{";
        echo jsonn("result", 0) . ",";
        echo jsons("message", "Could't query db");
        echo "}";
        return;
    }

    if ($row = $debt_obj->fetch()) {

        echo "{";
        echo jsonn("result", 1) . ",";
        echo jsons("student_id", $row["student_id"]) . ",";
        echo jsons("student_name", $row["student_name"]) . ",";
        echo jsons("amount_owed", $row["amount_owed"]) . ",";
        echo jsons("amount_paid", $row["amount_paid"]);
        echo "}";
        return;
    } else {
        echo "{";
